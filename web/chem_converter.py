...
def getCanonicalIsomerSmiles(smiles, stereo_value):
    # # 입체화학 선택 여부 확인
    # while True:
    #     stereo_choice = stereo_value
    #     if stereo_choice in ['y', 'n']:
...
def InChIKeyToSmiles(inchi_key):
...
def InchiToSmiles(inchi):
    ...
    try:
        ...
        print(f"Warning: SMILES generation failed. Error: {str(e)}")
        smiles = Chem.MolToSmiles(mol, canonical=False)
    return smiles

def sdfToSmiles(file_content):
    # 임시 파일에 파일 내용을 기록
    with tempfile.NamedTemporaryFile(suffix=".sdf", delete=False) as temp_file:
        temp_file.write(file_content)
        temp_file.flush()  # 파일 내용을 완전히 기록
        temp_file_path = temp_file.name  # 임시 파일 경로 저장
    
    try:
        # SDMolSupplier에 임시 파일 경로를 전달
        ...
        for mol in supplier:
            if mol is None:
                continue  # 파일 내 잘못된 분자를 건너뜀
            try:
                sdf_smiles = Chem.MolToSmiles(mol, canonical=False)
            except Exception as e:
                print(f"Warning: SMILES generation failed for SDF molecule. Error: {str(e)}")
                continue
...
def molToSmiles(file_content):
    # 파일이 바이너리일 경우 텍스트로 변환
    if isinstance(file_content, bytes):
        file_content = file_content.decode('utf-8')

    # 임시 파일에 파일 내용을 기록
    with tempfile.NamedTemporaryFile(delete=False, suffix=".mol2") as temp_file:
        temp_file.write(file_content.encode('utf-8'))  # 텍스트 데이터를 바이너리로 저장
        temp_file.flush()
        temp_file_path = temp_file.name  # 임시 파일 경로 저장

    try:
        # RDKit에서 MOL2 파일을 불러오기
        mol = Chem.MolFromMol2File(temp_file_path)  # MolFromMol2File 함수 사용

        if mol is None:
            raise ValueError("Invalid MOL2 file format")

        try:
            mol_smiles = Chem.MolToSmiles(mol, canonical=False)
        except Exception as e:
            print(f"Warning: SMILES generation failed. Error: {str(e)}")
            mol_smiles = Chem.MolToSmiles(mol, canonical=False)
...
def sdfDirToSmiles(sdf_directory, output_csv_path):
    data = []
            ...
            for mol in supplier:
                if mol is not None:
                    try:
                        ...
                        print(f"Warning: SMILES generation failed for SDF molecule in {filename}. Error: {str(e)}")
                        sdf_smiles = Chem.MolToSmiles(mol, canonical=False)
...
def molDirToSmiles(mol_directory, output_csv_path):
    data = []
        ...
        if filename.endswith('.mol'):
            ...
            if mol is not None:
                try:
                    ...
                    print(f"Warning: SMILES generation failed for MOL file {filename}. Error: {str(e)}")
                    mol_smiles = Chem.MolToSmiles(mol, canonical=False)
...
def iupacToSmiles(chemical_IUPAC):
    try:
        # Step 1: Try PubChem
